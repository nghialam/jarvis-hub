...
STRUCTURED_PROMPTS = {
    ...
    "jarvis-memory-compact": {
        "format": "/goal",
        "prompt": (
            "COMPACT weekly memory into lean persistent state for next week operations.\n"  
            "using all daily memory files from current month\n"
            "with constraints: remove anything older than 7 days unless critical, max 150 lines total\n"  
            "deliverable: updated MEMORY.md + compact report to Telegram chat showing what was removed/kept/prioritized"
        )
    },
    "jarvis-backlog-sync": {
        "format": "/goal",
    ...
    "jarvis-memory-daily": {
        "format": "/goal",  
        "prompt": (
            "UPDATE daily memory state based on today's sessions and any completed tasks.\n" 
            "using MEMORY.md + last 7 days of backlogs\n"  
            "with constraints: max 60 lines for today's log, keep persistent notes under 150 total lines\n"
            "deliverable: updated MEMORY.md + daily summary to Telegram chat with notable learnings and open items"  
        )
    },
    "ollama-daily-check": {
        "format": "/goal",  
        "prompt": (
            ...
            "using /api/tags endpoint + system memory diagnostics\n"  
    ...
    "jarvis-auto-improve": {
        ...
        "prompt": (
            ...
            "using system logs + MEMORY.md + backlogs\n"  
...
def run_parallel_chains(config):
    ...
    # Read config from structured_prompt system (Phase 1)  
    ...
    # In practice this becomes: results = spawn_parallel(chains_to_run, model="qwen3.6:35b-mlx")
...
def card_create(task_id, title, model="qwen3.6:35b-mlx"):
    conn = ensure_db()
