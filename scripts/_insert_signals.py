#!/usr/bin/env python3
"""Insert signals + alert feed API sections into app.py before main block."""
...
print(f"Will insert before line {insert_idx + 1} (main block)")

# Build the signals + alert feed section as raw string with perfect indentation
section_text = r'''# ====================================================================
...
# Insert section before main block
lines = content.split("\n")
main_idx = None
for i, line in enumerate(lines):
    if 'if __name__' in line and '__main__' in line:
...
if main_idx is None:
    print("ERROR: Could not find main block in app.py")
    import sys; sys.exit(1)

print(f"Appending {len(section_text.split(chr(10)))} lines before line {main_idx + 1} (main block)")

# Build new content with proper blank line separation
new_content = "\n".join(lines[:main_idx]) + "\n\n\n" + section_text + "\n\n\n" + "\n".join(lines[main_idx:])
